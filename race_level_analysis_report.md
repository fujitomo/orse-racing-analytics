...
**主要な成果**：
- グレードレベル（r=0.132, p<0.001）、レース場所レベル（r=0.106, p<0.001）、距離レベル（r=0.061, p<0.001）全てで統計的に有意な正の相関を確認
- 重み算出結果（グレード:0.540, 場所:0.344, 距離:0.116）が競馬のドメイン知識と完全に整合
- 賞金データに基づく客観的評価により、主観的判断を排除した科学的アプローチを実現
...
**分析対象**: 5,668頭（最低2戦以上の競走馬）
**分析期間**: 2010年1月〜2014年12月（50ファイル分）
...
- **分析結果**:
  ...
  - 効果サイズ: 小効果（Cohen基準）
  - 統計的有意性: 有意
- **解釈**: グレードレベルと複勝率の間には統計的に有意な正の相関関係が確認された。効果は小さいものの、レースの格付けが高いほど複勝率が向上する傾向が実証された。
...
- **分析結果**:
  ...
  - 効果サイズ: 小効果（Cohen基準）
  - 統計的有意性: 有意
- **解釈**: レース場所レベルと複勝率の間には統計的に有意な正の相関関係が確認された。格式の高い競馬場での競走経験が複勝率向上に寄与することが示された。
...
- **分析結果**:
  ...
  - 効果サイズ: 効果なし（Cohen基準）
  - 統計的有意性: 有意
- **解釈**: 距離レベルと複勝率の間には統計的に有意な相関関係が確認されたが、効果サイズは非常に小さく、実務的な予測力は限定的である。

#### 5.0.3. 重み算出結果

第3部の設計仕様 `w_i = r_i² / (r_grade² + r_venue² + r_distance²)` に基づき算出された重み：

- **グレードレベル**: w₁ = 0.540 (r = 0.132)
- **レース場所レベル**: w₂ = 0.344 (r = 0.106)  
- **距離レベル**: w₃ = 0.116 (r = 0.061)

**ドメイン知識との整合性確認**: 算出された重みの順序（グレード > 場所 > 距離）は、競馬の専門的知見と完全に一致しており、設計の妥当性が実証された。
...
#### 5.1.1. 年齢層別分析
年齢層別によるレースレベルと複勝率の関係分析を実施し、以下の知見を得た。

**主要知見:**
- 若馬（2-3歳）では距離適性の影響が大きい
- 古馬（4歳以上）ではグレード経験の効果が顕著  
- 年齢を重ねるごとに平均レースレベルと複勝率の相関が強化

#### 5.1.2. 経験数別分析
出走経験数別によるレースレベルと複勝率の関係分析を実施し、キャリア段階による特徴量効果の違いを確認した。

**主要知見:**
- 経験豊富な馬ほど高グレードレースでの安定性向上
- 初期キャリア（1-10戦）では場所レベルの影響が大きい
- 中堅期（11-30戦）以降はグレードレベルの重要性が増加

#### 5.1.3. 距離カテゴリ別分析
距離カテゴリ別によるレースレベルと複勝率の関係分析を実施し、距離適性による効果の差異を明確化した。

**主要知見:**
- 短距離（1000-1400m）では場所レベルの影響が大きい
- 中距離（1801-2000m）では最もバランスの取れた特徴量効果
- 長距離（2401m以上）ではグレードレベルの重要性が増加

### 5.2. 時系列安定性評価

#### 5.2.1. 平均レースレベルの安定性
2010-2024年（5期間）にわたる時系列分析により、特徴量の安定性を検証した。

- **平均相関係数**: 0.700
- **標準偏差**: 0.016
- **変動係数**: 0.023 (2.3%)
- **変動範囲**: 0.039 (0.684 - 0.723)

**評価**: 変動係数2.3%は統計的に十分な安定性を示しており、経時的な特徴量の妥当性が確認された。

#### 5.2.2. 最高レースレベルの安定性
最高レースレベルについても同様の安定性評価を実施した。

- **平均相関係数**: 0.460
- **標準偏差**: 0.017
- **変動係数**: 0.037 (3.7%)
- **変動範囲**: 0.040 (0.442 - 0.482)

**評価**: 最高レースレベルも平均レベルと同様に安定した予測性能を維持している。
...
**平均レースレベルの性能:**
- **平均R²**: 0.491
- **競馬ドメイン評価**: 中程度（moderate）
- **安定性**: ±0.023
- **実用性**: 補助指標として有用、単独使用は不適切

**最高レースレベルの性能:**
- **平均R²**: 0.220
- **競馬ドメイン評価**: 限定的（limited）
- **安定性**: ±0.017
- **実用性**: 参考程度、予測精度は期待できない

#### 5.3.2. 競馬特化型ベンチマーク基準
競馬業界の現実的制約と先行研究を踏まえた評価基準：

- **実用レベル (Practical)**: R² ≥ 0.65（他の主要特徴量との組み合わせで商用レベル）
- **中程度 (Moderate)**: R² ≥ 0.45（補助指標として価値あり）
- **限定的 (Limited)**: R² ≥ 0.25（統計的意味はあるが実用性は低い）
- **効果微小 (Minimal)**: R² < 0.25（学術的興味のみ）

**競馬ドメインにおける現実的評価**: 
- 平均レースレベル（R² = 0.491）は「中程度」に位置し、**単独での実用化は困難だが、他の予測変数との組み合わせにより価値を発揮する可能性がある**
- 競馬予測では完璧な予測は不可能であり、R² = 0.5前後でも相応の価値を持つが、過度な期待は禁物である
...
本分析で得られたR²=0.491という決定係数は、競走馬の複勝率変動の約半分しか説明できていない。これは以下の要因によるものと考えられる：

1. **本質的不確実性**: 競馬は多数の確率的要因（馬場状態、騎手判断、馬の体調等）が複合的に影響する確率的現象
2. **観測不可能要因**: データ化されていない要素（調教内容、厩舎環境、馬の気性等）の存在
3. **特徴量設計の限界**: 過去実績のみに基づく評価で、現在の能力状態を十分反映できない

**統計学的解釈**: Cohen (1988)の効果サイズ基準では、R²=0.491は「大きな効果」に分類されるが、予測精度としては中程度に留まる。競馬予測の文脈では、完全予測が原理的に不可能であることを考慮すると、統計的には意味のある関連性を示していると評価できる。

### 5.7. 結論

HorseRaceLevelは、競走馬の複勝率予測において統計的に有意な関連性を示す特徴量として検証されたが、実用性には重要な制約が存在する。

**検証された事実:**
1. **中程度の説明力**: R² = 0.491により約半分の変動を説明（残り半分は未説明）
2. **時系列安定性**: 5期間にわたる一貫した性能維持
3. **理論的整合性**: ドメイン知識との論理的一致
4. **技術的実装性**: システム組み込みの技術的障壁は低い

**現実的な価値評価:**
- **補助指標としての価値**: 他の主要特徴量との組み合わせ使用で限定的効果
- **単独使用の不適切性**: この指標のみでの予測は信頼性に欠ける
- **競馬予測の本質的困難性**: 高い不確実性は本特徴量でも根本的解決に至らず

**学術的意義と実務への示唆:**
本研究は特徴量エンジニアリングの方法論としては価値があるが、競馬予測の実用化には更なる多面的アプローチが必要であることを示している。データサイエンスにおいて、統計的有意性と実用的価値は別次元の課題であることを実証する事例としても意味を持つ。
