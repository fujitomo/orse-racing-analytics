...
**主要な成果**：
- **実用的な予測性能**: Out-of-Time検証において決定係数R²=0.060を達成し、競馬予測の単一特徴量として実用レベルの性能を確認。
- **統計的有意性の確認**: 3要素すべてで統計的に有意な正の相関関係を確認（p < 0.001）。
- **芝レース特化の重み配分**: 相関強度に基づく重み付け（グレード61.8%, 場所33.7%, 距離4.5%）により、芝レースにおける場所要因の重要性を定量化。
- **厳密な時系列分割**: 訓練期間（2010-2020年）と検証期間（2021-2022年）を完全分離し、データリーケージを防止。
- **統計的妥当性**: マルチコリニアリティ検証（VIF < 5.0）により、特徴量間の適切な独立性を確認。
...
**主目標**：
各競走馬がキャリアを通じて経験してきた **「競走環境の厳しさ」**と複勝率の関係を調査するのが目的。  
その為、**「競走環境の厳しさ」**を客観的かつ定量的に評価する。  
具体的には複数の特徴量を重みづけ等行い統合した**合成特徴量** を構築する。その予測有効性を統計的に検証すること。

**副目標**：

1. 1. レースの水準を定量化した指標（HorseRaceLevel）を作成し、JRDBのタイム指数・成績率（勝率・複勝率）・総合評価指数（例：IDM）と比較して、より高精度な予測を可能にする特徴量を開発する
...
- **データソース**: JRDB 成績データ（SED,SRB,BAC）　 TODO：データソースの説明を追加
 - **対象期間**: 2010 年 1 月 1 日〜2025 年 12 月 31 日（芝レース限定）
- **データ概要**:
...
- グレードレベル: w₁ = 0.635 (63.5%)
- レース場所レベル: w₂ = 0.322 (32.2%)
- 距離レベル: w₃ = 0.043 (4.3%)
...
- **訓練期間 (In-Sample):** `2010-2020年` (150,958件, 約69%)
  - **目的:** 特徴量の重み（w）や補正係数など、モデルの全パラメータを**この期間のデータのみを用いて**決定します。
- **検証期間 (Out-of-Sample):** `2021-2022年` (32,532件, 約15%)
  - **目的:** 訓練期間で決定したパラメータを**一切変更せず**に、モデルの予測性能が将来の未知データに対しても維持されるか（汎化性能）を評価します。
- **テスト期間 (Out-of-Sample):** `2023-2025年` (34,703件, 約16%)
...
実際の適用式（実装ベース）:
`RacePoint = (0.635 * グレードレベル) + (0.322 * レース場所レベル) + (0.043 * 距離レベル)`
...
- **📊 統計的特性:**
  ...
  - **適度な汎化性能**: 未知データに対して現実的な性能を維持（汎化性能44.6%）。
...
本研究で開発された合成特徴量`HorseRaceLevel`は、芝レースを対象とした競走馬の複勝率予測において、統計的に有意で実用的な価値を持つ特徴量であることが、厳密な時系列検証を通じて確認された。

**【検証された事実】**
1. **実用的な予測性能**: 検証期間において、単一の特徴量として決定係数R²=0.060を達成し、競馬予測の分野で実用レベルの説明力を示した。
...
**【実務的価値評価】**
- **補助特徴量として有用**: R²=0.060という実用的な性能から、本特徴量は他の指標と組み合わせることで予測精度の向上に寄与する。
- **芝レース専門分析への価値**: 芝レース特化により、より専門性の高い分析が可能となった。
...
総じて、本研究は、競馬予測における特徴量エンジニアリングの実用例として、データサイエンス分野の実務応用に寄与する価値を持つ。
