...
**研究の結果**：
- **予測性能**: Out-of-Time検証における決定係数R²=0.051、相関係数r=0.226
...
- **決定係数**: R² = 0.051（n=3,119頭）
- **相関係数**: r = 0.226（Pearson）
- **統計的有意性**: p < 0.001
- **効果サイズ**: 小効果（Cohen基準）
...
#### 5.0.2. 実装結果と固定重み配分（データリーケージ防止設計）
...
- **訓練期間相関（2010-2020年データ）**:
  ...
  - 決定係数: R² = 0.124
  - 効果サイズ: 中効果（Cohen基準）
  - 解釈: グレードレベルは、複勝率と統計的に有意で中程度の正の相関を持つことが確認された。　　
  これは`HorseRaceLevel`の最も重要な構成要素である。
...
- **訓練期間相関（2010-2020年データ）**:
  ...
  - 決定係数: R² = 0.063
  - 効果サイズ: 小効果（Cohen基準）
  - 解釈: レース場所レベルは、複勝率と統計的に有意な正の相関を持つことが確認された。馬場の格式が要因として小効果であることを示している。
...
- **訓練期間相関（2010-2020年データ）**:
  ...
  - 決定係数: R² = 0.008
  - 効果サイズ: 効果なし（Cohen基準）
  - 解釈: 距離レベルと複勝率の相関は統計的に有意だが効果量は微小である。　　
...
# 固定重み（訓練期間算出済み）
FIXED_WEIGHTS = {
    'grade_weight': 0.618,   # 61.8%
    'venue_weight': 0.337,   # 33.7% 
    'distance_weight': 0.045 # 4.5%
}

# RacePoint計算（全期間でこの式を使用）
RacePoint = (0.618 * グレードレベル) + (0.337 * レース場所レベル) + (0.045 * 距離レベル)
```

**実装の特徴**:
- 重みは訓練期間（2010-2020年）で一度だけ算出
- 検証・テスト期間では重みの再計算は一切実施しない
...
- **検証期間（Out-of-Sample）性能**: r = 0.226, R² = 0.051 (n=3,119)

**評価**:
- **適度な汎化性能**: 訓練期間から検証期間への性能変化は41.2%（R²: 0.124→0.051）であり、過学習を避けた現実的な汎化能力を示している。
...
- **時系列分割（Out-of-Time検証）**: r = 0.226, R² = 0.051（検証期間: 3,119頭）
...
- **R² = 0.051**: 実際の予測で期待できる現実的な性能（実用的評価）

**実務的意義:**
実用的な競馬予測システムとしては、**R² = 0.051が正確な評価値**であり、単一特徴量としては十分実用的な性能を示している。この数値は、データリーケージを完全に防止した厳密な検証により得られた信頼性の高い指標である。
...
**統合特徴量（RacePoint）の性能:**
- **検証期間R²**: **0.051** (n=3,119頭)
- **検証期間相関係数**: **0.226**
- **p値**: < 0.001（統計的有意）
- **Cohen基準評価**: 小効果（r=0.226）
- **競馬ドメイン評価**: **実用的**
...
**【検証された事実】**
1. **統計的関連性の確認**: 検証期間において決定係数R²=0.051（相関係数r=0.226、p<0.001）を達成し、競馬予測における補助特徴量として一定の説明力を示した。
2. **芝レース特化の3要素統合**: グレード・場所・距離の3要素を、相関強度に基づく客観的な重み（グレード:61.8%, 場所:33.7%, 距離:4.5%）で統合し、芝レースにおける場所要因の重要性を定量化した。
3. **適切な汎化能力**: 厳密なOut-of-Time検証により、データリーケージを防止し、訓練期間（R²=0.124）から検証期間（R²=0.051）への現実的な汎化性能を確認した。
...
**【実務的価値評価】**
- **補助特徴量としての価値**: R²=0.051（相関係数r=0.226）の統計的関連性から、本特徴量は他の指標と組み合わせることで分析の多角化に寄与する。
